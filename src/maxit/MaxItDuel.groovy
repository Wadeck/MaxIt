...
/**
 * Use to compare two IA
 * @author Wadeck
 */
@CompileStatic
public class MaxItDuel {
...
//	private Map<Closure, PlayerDuelStats> stats
	private Map stats
...
//	@CompileStatic(TypeCheckingMode.SKIP)
	def displayResult(){
		for (Entry e : stats.entrySet()) {
			...
			PlayerDuelStats ps = (PlayerDuelStats)(e.value)
...
//		println '---'
	...
	def oneGame(Closure pf1, Closure pf2, int seed){
		...
		((PlayerDuelStats)stats[pf1]).addStatVs(pf2, logic.getScoreH(), logic.getScoreV())
		((PlayerDuelStats)stats[pf2]).addStatVs(pf1, logic.getScoreV(), logic.getScoreH())
...
//				                    NStepPlayer,
//				                    ]
		new MaxItDuel(participants).start()
...
class PlayerDuelStats{
	public Closure p
	Map<Closure, List<Integer>> scores
	def PlayerDuelStats(Closure p){
		this.p = p
		this.scores = [:]
	}
